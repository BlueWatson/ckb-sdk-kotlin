...
class Api @JvmOverloads constructor(nodeUrl: String, isDebug: Boolean = false) {
    ...
    fun getTipHeader(): Header? {
        return rpcService.post<Header>("get_tip_header", emptyList<String>(), Header::class.java)
    }

    fun getTipBlockNumber(): BigInteger? {
        val blockNumber = rpcService.post<String>("get_tip_block_number", emptyList<String>(), String::class.java)
        return blockNumber?.let { Numeric.toBigInt(it) }
    }

    fun getCurrentEpoch(): Epoch? {
        return rpcService.post<Epoch>("get_current_epoch", emptyList<String>(), Epoch::class.java)
    }

    /** Stats RPC  */
    fun getBlockchainInfo(): BlockchainInfo? {
        return rpcService.post<BlockchainInfo>("get_blockchain_info", emptyList<Any>(), BlockchainInfo::class.java)
    }

    fun getPeersState(): List<PeerState>? {
        return rpcService.post<List<PeerState>>(
            "get_peers_state", emptyList<Any>(), object : TypeToken<List<PeerState>>() {}.type
        )
    }

    fun getBannedAddress(): List<BannedResultAddress>? {
        return rpcService.post<List<BannedResultAddress>>(
            "get_banned_address",
            emptyList<Any>(),
            object : TypeToken<List<BannedResultAddress>>() {}.type
        )
    }

    fun getPeers(): List<NodeInfo>? {
        return rpcService.post<List<NodeInfo>>(
            "get_peers", emptyList<Any>(), object : TypeToken<List<NodeInfo>>() {}.type
        )
    }

    fun getLockHashIndexStates(): List<LockHashIndexState>? {
        return rpcService.post<List<LockHashIndexState>>(
            "get_lock_hash_index_states",
            emptyList<Any>(),
            object : TypeToken<List<LockHashIndexState>>() {}.rawType
        )
    }
    ...
    fun getCellsByLockHash(
        ...
        return rpcService.post<List<CellOutputWithOutPoint>>(
            ...
            object : TypeToken<List<CellOutputWithOutPoint>>() {}.type
        )
    }
    ...
    fun getLiveCellsByLockHash(
        ...
        return rpcService.post<List<LiveCell>>(
            ...
            object : TypeToken<List<LiveCell>>() {}.type
        )
    }
    ...
    fun getTransactionsByLockHash(
        ...
        return rpcService.post<List<CellTransaction>>(
            ...
            listOf(lockHash, Numeric.toHexString(page), Numeric.toHexString(pageSize), reverseOrder),
            object : TypeToken<List<CellTransaction>>() {}.type
        )
    }
}
